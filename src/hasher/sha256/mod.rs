...
use super::Hasher;
use error::E;
use sha2::{Digest, Sha256 as Origin};
...
impl Default for Sha256 {
    /// Creates a default instance of `Sha256` hasher.
    fn default() -> Self {
        Sha256 {
...
impl Sha256 {
    /// Creates a new instance of `Sha256` hasher.
    pub fn new() -> Self {
        Sha256 {
            hasher: Some(Origin::new()),
            hash: None,
...
mod test {
    ...
    fn correction_buffering() -> Result<(), E> {
        ...
        utils::compare_same_dest::<hasher::sha256::Sha256, reader::buffering::Buffering>(
            &usecase, None,
        )?;
        usecase.clean()?;
        Ok(())
    ...
    fn changes_buffering() -> Result<(), E> {
        let usecase = UseCase::unnamed(2, 2, 2, &[])?;
        utils::check_for_changes::<hasher::sha256::Sha256, reader::buffering::Buffering>(
            &usecase, None,
        )?;
        usecase.clean()?;
        Ok(())
    ...
    fn correction_mapped() -> Result<(), E> {
        let usecase = UseCase::unnamed(2, 2, 2, &[])?;
        utils::compare_same_dest::<hasher::sha256::Sha256, reader::mapping::Mapping>(
            &usecase,
    ...
    fn changes_mapped() -> Result<(), E> {
        let usecase = UseCase::unnamed(2, 2, 2, &[])?;
        utils::check_for_changes::<hasher::sha256::Sha256, reader::mapping::Mapping>(
            &usecase,
            Some(ReadingStrategy::MemoryMapped),
        )?;
        usecase.clean()?;
